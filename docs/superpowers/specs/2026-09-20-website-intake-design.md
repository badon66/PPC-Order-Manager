...
**Date:** 2026-09-20
**Status:** approach chosen by Keenan ("Draft order with upload link"), spec awaiting his review
...
|---|---|---|
| `startingPoint` | yes — `Design ready` or `Starting from scratch` | `enquiry.startingPoint` |
...
- `requestClientDetails: true`, `clientLinkSections: { logos: true, inspiration: true, roster: false, personalDetails: false }` — logos and inspiration because that is what the two routes ask for; roster off because sizes come after the design; personal details off because they just gave them. Keenan flips any of the four on the order later and the same link updates.
...
2. The success panel on the website shows the upload button, the link opens the roster page for
   that Draft asking for logos and inspiration only, and an uploaded logo lands on that order.
