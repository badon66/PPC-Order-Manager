...
`scripts/build-call-template.py` (openpyxl; dev-time only). Pick-list values
...
- **Pure modules** with `node --test` (no runner to install; `npm test`
  script added): `import.ts` (fixture `.xlsx` and `.csv` covering aliases,
...
scripts/build-call-template.py
