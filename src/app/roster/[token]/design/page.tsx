...
const STAGE = 'design' as const;

/**
 * Customer-facing, same token as the hub. A dedicated entry point for the
 * design stage — logos and inspiration only — so an email pointing at "send
 * us your logos" doesn't land a customer on a page also asking for a roster
 * they already sent, or haven't touched.
 *
 * No login — the token in the URL is the credential, same as the hub.
 */
export default async function DesignStagePage({ params }: { params: Promise<{ token: string }> }) {
